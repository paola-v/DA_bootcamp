...
DROP SCHEMA IF EXISTS da_bootcamp_ignatova; 
CREATE SCHEMA IF NOT EXISTS da_bootcamp_ignatova;
Use da_bootcamp_ignatova;
...
-- 1.1 Create a new table 'animals' with an ID column (type INT) and 2 additional columns (any name and datatype).
...
-- 1.2 Add a new column to the table, then DROP one of the other columns (other than ID).
ALTER TABLE animals
	ADD COLUMN age INT; 
ALTER TABLE animals
	DROP COLUMN type;

-- 1.3 Rename the table to 'zoo_animals'
ALTER TABLE animals
RENAME TO zoo_animals;

-- 1.4 Select all records from the (empty) zoo_animals table
SELECT * FROM zoo_animals;

-- 1.5  Drop the table - actually, to be on the safe side, use the "if exists" condition to drop animals if it exists, and drop zoo_animals if it exists.
...
-- 2.1. Create table item_details using field names (item_id, item_price, item_description), using sensible data types.
...
-- 2.2 INSERT all raw data provided into item_detail. COMMIT the change.
...
-- 2.3 SELECT * to confirm all 5 rows are present.
SELECT * FROM item_details;


-- 2.4. DELETE all records for which the item price is greater-than-or-equal-to 2. 
DELETE FROM item_details
WHERE item_price>=2.00;


-- 2.5. SELECT * to confirm deletions. ROLLBACK the change, and SELECT * once again to confirm 5 original rows present.
SELECT * FROM item_details;
ROLLBACK;
SELECT * FROM item_details;


-- 2.6. Ah, the price for Apples was wrong. UPDATE that record to price 6 instead of 60.
UPDATE item_details
SET item_price = 6.00 
WHERE item_description LIKE 'Apple%';


-- 2.7. The price of Avocados and Broccoli have doubled. UPDATE those records to reflect the price doubling.
UPDATE item_details
SET item_price =2*(item_price)
WHERE item_id IN ('itm001','itm005');


-- 2.8. SELECT * to confirm the records are correct. COMMIT. Table item_details is ready for use in PROBLEM 3.
SELECT * FROM item_details;
COMMIT;


-- 2. 9. SUGGESTION: To make debugging PROBLEM 3 easier, you may wish to make note of item_details record values as of Step 8, then TRUNCATE item_details and INSERT records to match those recorded values to have a reliable starting point.
...
-- PROBLEM 3 SOLUTION
/* 3.1 Create table sales_orders with 5 columns: 
		a. record_id, a Big Integer Primary Key that auto-increments with each new record added.
        b. order_no, another Big Integer that cannot be NULL
        c. order_date, DATE
        d. item_id, formatted to match item_details.item_id from PROBLEM 2.
...
-- 3.2 	Load sales_orders with the raw data provided in pg2. You may write and leave the entire INSERT statement there.
INSERT INTO sales_orders (order_no,order_date,item_id,quantity)
VALUES
...
-- 3.3. What is the total number of records in sales_orders? 
SELECT COUNT(*) FROM sales_orders;
-- ANSWER: There are 831 records.

-- 3.4. What is the total number of records of sales_orders INNER JOIN item_details? LEFT JOIN?
SELECT COUNT(*) 
FROM sales_orders
INNER JOIN item_details
	ON sales_orders.item_id=item_details.item_id;
-- ANSWER: There are 828 records.
...
-- 3.5  Return the order_no, order_date, and the order_total for the top 10 orders in August in descending order.
SELECT 
	s.order_no,
    s.order_date,
    SUM(s.quantity*i.item_price) AS order_total
FROM sales_orders s
...
-- 3.6 Return the order_no, order_date, and the total quantity for orders HAVING a total quantity greater than 10.
SELECT 
	order_no,
    order_date,
...
-- 3.7 Create stored procedure "total_sales_on_date" that returns total sales (in $) given a date.

DELIMITER //
CREATE PROCEDURE total_sales_on_date (IN date_input DATE)
BEGIN
	SELECT 
		s.order_date,
        CONCAT('$',' ',SUM(s.quantity*i.item_price)) AS total_sales
	FROM sales_orders s
	INNER JOIN item_details i
	ON i.item_id = s.item_id
	GROUP BY s.order_date
    HAVING s.order_date=date_input;
 END //
...
/* I started with listing start date 2019-07-01 in cell A1 and end date 2019-12-31 in cell A2.
 Then in cell C1, I called the sequence function to generate a sequence array of numbers between these two dates. 
 =SEQUENCE(1+A2,1,A1)
 This resulted in an array that looked like this (43647,43648,43649....87477).
 In cell D1 then I used the text function to converte these to the YYYY-MM_DD format. I then added the paranthesis and the single quotation marks to prep them for the insert statement in SQL
=TEXT(C1,"('YYYY-MM-DD')") and I used auto fill handle to populate these to the end. Then I used the =A1&"," function to add the comma to the paranthesis, selected everything and pasted it as values. 
 */
...
SELECT 
    ...
    CONCAT('$',' ',(s.quantity*i.item_price)) AS total_sales
...
SELECT 
	item_id,
    CONCAT(YEAR(order_date),'Q', QUARTER(order_date)) AS Quarter_YYYYMM, 
	SUM(quantity) AS Total_Quantity
	FROM sales_orders
GROUP BY item_id, Quarter_YYYYMM
ORDER BY 2,1 ASC;
